...
test('paddingStart offsets the first row and first column by 100px', async ({
  page,
}) => {
  await page.goto('/padding')
...
test('vertical and horizontal lists re-window on scroll', async ({ page }) => {
  await page.goto('/padding')
  await expect(
    page.locator('[data-testid="rows-list"] [data-index="0"]'),
  ).toBeVisible()
  await page.locator('[data-testid="rows-list"]').evaluate((el) => {
    el.scrollTop = 5000
  })
  await expect(
    page.locator('[data-testid="rows-list"] [data-index="0"]'),
  ...
  await page.locator('[data-testid="cols-list"]').evaluate((el) => {
    el.scrollLeft = 5000
  })
  await expect(
    page.locator('[data-testid="cols-list"] [data-index="0"]'),
  ).toHaveCount(0)
  await expect(
    page.locator('[data-testid="cols-list"] [data-index]').first(),
  ).toBeVisible()
})
...
test('indexAttribute keeps the two grid virtualizers apart under 2D scrolling', async ({
  ...
  const anyCell = page
    .locator('[data-testid="grid-list"] [data-row-index]')
    .first()
...
test('toggle unmounts and remounts the grid; it keeps working after', async ({
  page,
}) => {
  await page.goto('/padding')
