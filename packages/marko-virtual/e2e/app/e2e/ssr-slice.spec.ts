...
test('server HTML contains the painted slice: first rows present, deep rows absent, real total size', async ({
  ...
  const res = await request.get('/ssr-slice')
...
test('with JavaScript disabled the painted rows exist in the DOM (streaming swap is JS-dependent)', async ({
  ...
  // content OUT-OF-ORDER and swaps it into place with a tiny inline script. With JS
  // disabled that swap never runs, so the server-painted rows sit complete in the
  // DOM (the view-source test proves they are in the HTML) while the reader is still
  // looking at the placeholder. This characterizes the platform contract:
  // out-of-order <await> pages are not no-JS pages; script-less VISIBLE rows would
  // require in-order rendering (no placeholder).
  const context = await browser.newContext({ javaScriptEnabled: false })
  const page = await context.newPage()
  await page.goto('/ssr-slice')
  expect(
    await page.locator('.email', { hasText: 'person1@example.com' }).count(),
  ).toBe(1)
  expect(await page.locator('.item').count()).toBeGreaterThan(5)
  // The placeholder is what stays on screen: the swap that would replace it is the
  // part that needs JS. (The dev server hides it instead — this asserts the
  // production behavior, which is the one users get.)
  await expect(page.getByText('Loading people')).toBeVisible()
  await context.close()
})
...
test('the awaited slice STREAMS: placeholder flushes first, server-painted rows arrive in a later chunk', async ({
  ...
  const response = await fetch(`${baseURL}/ssr-slice`)
