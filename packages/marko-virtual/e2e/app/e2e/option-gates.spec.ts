...
test('debug=true does not disturb rendering or windowing', async ({ page }) => {
  // The log itself is NOT asserted here. Core writes it from a
  // `process.env.NODE_ENV !== 'production'` branch, so it does not exist in the
  // production build this suite runs against — and a console.info assertion needs
  // no browser anyway. That proof lives in the vitest tier
  // (tests/options.test.ts, "debug=true makes core emit its timing logs").
  // What is worth proving in a real browser is that turning the option on does not
  ...
  await page.waitForFunction(() => !document.querySelector('[data-index="0"]'))
  // Re-windowed to the scrolled region rather than stalling or blanking.
  await expect(page.locator('[data-index="50"]')).toBeVisible()
})
