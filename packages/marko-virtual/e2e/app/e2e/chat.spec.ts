...
async function scrollTop(page: Page): Promise<number> {
  return page.locator('.messages').evaluate((el) => el.scrollTop)
}

// Waits until scrollTop stops moving on its own. After a programmatic jump the
// virtualizer measures the rows that just came into view, and a measured size above
