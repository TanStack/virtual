...
test('reserves all 500 slots upfront while the first page loads', async ({ page }) => {
  await page.goto('/')
  ...
  await expect(page.locator('.item', { hasText: /Row #1\b/ }).first()).toBeVisible()
  await expect(page.getByText('20 of 500 rows loaded')).toBeVisible()
})

test('scrolling to the edge of loaded rows fetches the next page', async ({ page }) => {
  await page.goto('/')
  await expect(page.getByText('20 of 500 rows loaded')).toBeVisible()
  ...
  await expect(page.locator('.item', { hasText: /Row #21\b/ }).first()).toBeVisible()
})
...
test('a deep jump shows placeholders, then pages load continuously until the region catches up', async ({
  ...
  await expect(page.locator('.item', { hasText: /Row #401\b/ }).first()).toBeVisible({
    timeout: 20_000,
  })
})
